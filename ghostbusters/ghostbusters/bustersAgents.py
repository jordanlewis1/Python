...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions:
            tPos = dist.argMax()
            "temperary Pos"
            tDistance = self.distancer.getDistance(pacmanPosition, tPos)
            "temperary Dist is the dist between pacmanPosition and tPos"
            if tDistance < distance:
                "if distance if greater than temp Dist"
                distance = tDistance
                ghostPos = tPos
                
        dist = float("inf")
        act = None
        "actions is set to None"
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            tDistance = self.distancer.getDistance(successorPosition, ghostPos)
            if tDistance < distance:
                distance = tDistance
                act = a
        return act

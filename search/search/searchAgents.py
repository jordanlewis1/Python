...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.been = tuple([False, False, False, False])
    ...
    def getStartState(self):
        ...
        xy = self.startingPosition
        self.been = self.myFunction()
        cornerUp = (xy, self.been)
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            nState = self.getNextState(state, action)
            cost = self.getActionCost(state, action, nState)
            children.append((nState, action, cost))
    ...
    def getUpdatedCorners(self, x, y, corners):
    ...
    def getNextState(self, state, action):
        ...
        m = self.getUpdatedCorners(nextx, nexty, state[1])
        
        #util.raiseNotDefined()
        # you will need to replace the None part of the following tuple.
        return ((nextx, nexty), m)
...
def cornersHeuristic(state, problem):
  ...
  # Default to trivial solution
    ...
    for i in range(len(corners)):
        #x and y positions and corners
        mHeurustic = abs(state[0][0] - corners[i][0]) + abs(state[0][1] - corners [i][1])
        if state [1][i] == False:
            cost = max(mHeurustic, cost)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    heurustic = 0
    foodC = foodGrid.asList()
    distances = []
    
    for coord in foodC:
        distances.append(mazeDistance(position, coord, gameState = problem.startingGameState))

    if len(distances) > 0:
        heurustic = max(distances)
    else:
        heurustic = 0
